...
user="admin"
# Generate password that meets TAK complexity: min 15 chars, 1 upper, 1 lower, 1 number, 1 special
# BusyBox-compatible password generation
pwd_base=$(cat /dev/urandom | tr -dc "a-z0-9" | dd bs=1 count=11 2>/dev/null)
pwd_upper=$(cat /dev/urandom | tr -dc "A-Z" | dd bs=1 count=1 2>/dev/null)
pwd_lower=$(cat /dev/urandom | tr -dc "a-z" | dd bs=1 count=1 2>/dev/null)
pwd_num=$(cat /dev/urandom | tr -dc "0-9" | dd bs=1 count=1 2>/dev/null)
pwd_special="!"
password="${pwd_base}${pwd_upper}${pwd_lower}${pwd_num}${pwd_special}"

## Set postgres password and ensure it meets validation criteria
# Generate password that meets TAK complexity: min 15 chars, 1 upper, 1 lower, 1 number, 1 special
pgpwd_base=$(cat /dev/urandom | tr -dc "a-z0-9" | dd bs=1 count=11 2>/dev/null)
pgpwd_upper=$(cat /dev/urandom | tr -dc "A-Z" | dd bs=1 count=1 2>/dev/null)
pgpwd_lower=$(cat /dev/urandom | tr -dc "a-z" | dd bs=1 count=1 2>/dev/null)
pgpwd_num=$(cat /dev/urandom | tr -dc "0-9" | dd bs=1 count=1 2>/dev/null)
pgpwd_special="!"
pgpassword="${pgpwd_base}${pgpwd_upper}${pgpwd_lower}${pgpwd_num}${pgpwd_special}"
...
#read -p "Enter the amount of memory to allocate, in kB. Default 6000000 (6GB): " mem
if [ -z "$mem" ];
then
	mem="6000000"
fi
...
printf $info "Waiting for TAK server to be ready. This may take 1-2 minutes...\n"
#$DOCKER_COMPOSE start tak
sleep 20  # Give TAK server more time to initialize services
...
do
	...
	# Try usermod - check for success in output, not just exit code
	# Ignore Ignite errors as they're expected during startup
	USERMOD_OUTPUT=$($DOCKER_COMPOSE exec -T tak bash -c "cd /opt/tak/ && java -jar /opt/tak/utils/UserManager.jar usermod -A -p $password $user" 2>&1)
	
	if echo "$USERMOD_OUTPUT" | grep -q "User.*Added\|User.*Updated"; then
		# Try certmod
		CERTMOD_OUTPUT=$($DOCKER_COMPOSE exec -T tak bash -c "cd /opt/tak/ && java -jar utils/UserManager.jar certmod -A certs/files/$user.pem" 2>&1)
		
		# certmod outputs "User Updated:" followed by "Fingerprint:" when successful
		if echo "$CERTMOD_OUTPUT" | grep -q "User.*Updated\|Fingerprint.*:"; then
			sleep 3  # Brief pause before schema upgrade
			
			# Try SchemaManager upgrade - check for success message
			SCHEMA_OUTPUT=$($DOCKER_COMPOSE exec -T tak bash -c "java -jar /opt/tak/db-utils/SchemaManager.jar upgrade" 2>&1)
			
			if echo "$SCHEMA_OUTPUT" | grep -q "up to date\|Successfully applied"; then
				printf $success "Database schema updated successfully\n"
				break
			else
				printf $info "SchemaManager not ready yet, retrying... ($RETRY/$MAX_RETRIES)\n"
			fi
		else
			printf $info "certmod not ready yet, retrying... ($RETRY/$MAX_RETRIES)\n"
		fi
	else
		printf $info "usermod not ready yet (Ignite service initializing), retrying... ($RETRY/$MAX_RETRIES)\n"
	fi
done

if [ $RETRY -ge $MAX_RETRIES ]; then
	printf $warning "Reached maximum retries ($MAX_RETRIES). Some operations may have failed, but continuing...\n"
fi
